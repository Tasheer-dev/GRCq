...
function printFullReport() {
  ...
  let html = `
    ...
    <head>
      <meta charset="UTF-8" />
      <title>تقرير تقييم الالتزام – قطاع التكنولوجيا والتحول الرقمي – تأشـيـر</title>
    ...
    <body>
      ...
      <div class="cover">
        ...
        <div class="cover-sub">تقرير التقييم الذاتي للالتزام بالتشغيل والحوكمة التقنية</div>
        <div class="cover-sub">إدارة التكنولوجيا والتحول الرقمي</div>
        <div class="cover-date">تاريخ الإصدار: ${today}</div>
      </div>
  ...
  sections.forEach(sec => {
    ...
    if (!sum) return;
    html += `<li>${sec} <span>– إجمالي الأسئلة: ${sum.total}, نسبة "نعم": ${sum.yesPct}%</span></li>`;
  });

  html += `</ul>`;

  // Each section with summary + table
  ...
  // Optional: you could also append narrative / open questions at the end:
  if (Object.keys(state.open || {}).length > 0) {
    html += `<div class="page-break"></div>`;
    html += `<h2 class="section-title">إجابات الأسئلة الوصفية الإضافية</h2>`;
    openQuestions.forEach(item => {
      const val = (state.open && state.open[item.key]) || "";
      if (!val) return;
      html += `
        <h3 style="margin-top:16px;font-size:15px;">${item.label}</h3>
        <p style="font-size:13px;white-space:pre-line;">${val}</p>
      `;
    });
  }
  ...
  html += `
      <div class="footer">
        <span>تم التوليد بواسطة نظام التقييم الذاتي – تأشـيـر</span>
        <span>${today}</span>
      </div>
    </body>
    </html>
